...
const parseLine = line => {
    const match = line.match(/(.+) (inc|dec) (.+) if (.+)/);
    return {
        reg: match[1],
        op: match[2],
        value: parseInt(match[3], 10),
        cond: parseCondition(match[4]),
    };
};
...
const condition = ({ reg, comparison, value }, registers) => {
    const regValue = registers[reg] || 0;
    // prettier-ignore
    switch (comparison) {
        case '>': return regValue > value;
        case '<': return regValue < value;
        case '>=': return regValue >= value;
        case '<=': return regValue <= value;
        case '==': return regValue == value;
...
const exec = instructions => {
    const registers = {};
    highwater = Number.NEGATIVE_INFINITY;

    instructions.forEach(inst => {
        registers[inst.reg] = registers[inst.reg] || 0;
        const value = registers[inst.reg];
        if (condition(inst.cond, registers)) {
            const newValue = operation[inst.op](value, inst.value);
            registers[inst.reg] = newValue;
            highwater = Math.max(highwater, newValue);
        }
    });
...
// const instructions = parse(test);
// const registers = exec(instructions);
// console.log(registers);
// console.log(maximum(registers));

const instructions = parse(input);
const registers = exec(instructions);
// console.log(registers);
console.log('max', maximum(registers)); // { name: 'jq', value: 4888 }
console.log('highwater', highwater); // 7774
