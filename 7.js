...
const depth = (name, nodes) => {
    return Math.max(
        0,
        ...nodes[name]
            .children
            .map(child => depth(child, nodes) + 1)
    );
};

const addDepth = nodes => {
    return Object
        .keys(nodes)
        .reduce((newNodes, name) => {
            newNodes[name] = Object.assign({}, nodes[name], {
                depth: depth(name, nodes),
            });
            return newNodes;
        }, {});
};
...
const deepestNode = nodes => {
    return Object
        .keys(nodes)
        .reduce(
            (max, name) => nodes[name].depth > max.depth ? { name, depth: nodes[name].depth } : max,
            { depth: -1 }
        );
};
...
const totalWeight = (name, nodes) => {
    return nodes[name].weight +
        sum(
            nodes[name]
                .children
                .map(child => totalWeight(child, nodes))
        );
};

const addTotalWeight = nodes => {
    return Object
        .keys(nodes)
        .reduce((newNodes, name) => {
            newNodes[name] = Object.assign({}, nodes[name], {
                totalWeight: totalWeight(name, nodes),
            });
            return newNodes;
        }, {});
};

// const unbalancedNode = nodes => {
//     return Object
//         .keys(nodes)
//         .filter(name => nodes[name].children);
// };

// console.log(load(test));
// console.log(addDepth(load(test)));
// console.log(deepestNode(addDepth(load(test))));

// console.log(addTotalWeight(load(test)));

console.log(deepestNode(addDepth(load(input)))); // { name: 'fbgguv', depth: 5 }
// console.log(unbalancedNode(addTotalWeight(load(input)))); // { name: 'fbgguv', depth: 5 }
